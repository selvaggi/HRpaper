...
\end{equation}
As a benchmark, we assume that the radius of the FCC inner tracking detector is $3/2$ that of the HE-LHC detector with a similar the magnetic field.  We also assume that the spatial resolution $\sigma_{r\phi}$ is 3 times smaller than at HE-LHC, which is possible by designing a more granular pixel detector~\cite{CMS:2012sda}. These specifications of the FCC detector would allow measurements of 1 TeV charged hadrons with a precision of $\sigma(p_T)/p_T  \simeq 2\%$, compared to $\sigma(p_T)/p_T \simeq 10\%$ for the HE-LHC detector. Central, high momentum charged hadrons tracks are assumed to be reconstructed with an efficiency $\epsilon = 95\%$.
...
Muons are also reconstructed using tracking. Stand-alone muon is provided by the angular difference between track angle in the muon system and the radial line connection to the beam axis. Again, assuming a 2 times better position resolution of in the muon system for the FCC detector, a combined muon momentum resolution of $\sigma(p_T)/p_T  \simeq 6\%$ can be achieved for momenta as high as 10 TeV/c, as opposed to $\sigma(p_T)/p_T \simeq 6\%$ at $\pt=3~TeV$ for the HE-LHC detector.
